...
class GlobuleBlanc(Agent):
    ...
    Comportement : algorithme de Boids (séparation, alignement, cohésion)
                   + chasse des virus (sol sombre et agents directs).
    Peinture sol : SOIL_WBC (blanc).
    """
    ...
    def _hunt(self, env: "Environnement", agents: List[Agent]) -> np.ndarray:
        ...
        - priorité 2 : centroïde du sol sombre dans le cône
        """
        visible_virus = [
            a for a in agents
    ...
    def paint_soil(self, env: "Environnement"):
        env.paint(self.pos[0], self.pos[1], SOIL_WBC)
